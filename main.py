...
def start_sorting():
    ...
    if sorting_algorithm:
        ...
        if sorting_algorithm == Sort.BUBBLE_SORT.value:
            Em.bubble_sort()
        if sorting_algorithm == Sort.INSERTION_SORT.value:
            Em.insertion_sort()
        if sorting_algorithm == Sort.SELECTION_SORT.value:
            Em.selection_sort()
        if sorting_algorithm == Sort.QUICK_SORT.value:
            Em.quick_sort()
        if sorting_algorithm == Sort.HEAP_SORT.value:
            Em.heap_sort()
        sorting_start_button.configure(text="Start", command=start_sorting, state='disabled', bg=COLOR_LIGHT_1)
...
element_shuffle_button = Button(ui_frame, text="Shuffle", font=40, bg=COLOR_DARK_2,
                                fg="White", command=shuffle_elements)
info_label_quantity = Label(ui_frame, text="< Quantity", bg=COLOR_DARK_1, fg=COLOR_LIGHT_2)
info_label_delay = Label(ui_frame, text="Delay >", bg=COLOR_DARK_1, fg=COLOR_LIGHT_2)
sorting_label = Label(ui_frame, text="Choose sorting algorithm:",
                      font=36, bg=COLOR_DARK_1, fg=COLOR_LIGHT_2)
style = ttk.Style()
...
sorting_delay_scale = Scale(ui_frame, from_=0.0, to=1.0, digits=3, resolution=0.01,
                            orient=HORIZONTAL, bg=COLOR_DARK_2, fg=COLOR_LIGHT_2)
...
info_label_quantity.place(relx=0.16, rely=0, relheight=0.2, relwidth=0.177)
info_label_delay.place(relx=0.68, rely=0, relheight=0.2, relwidth=0.177)
